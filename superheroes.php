...
if (isset($_GET['alias'])) {
    ...
    foreach ($superheroes as $superhero) {
        if ($alias === $superhero['alias']) {
            array_push($temp_superheroes, $superhero);
            break;
        }
    }
    //
...
?>
<?php if (!$hasRequestForSpecific) : ?>

    <ul>
        <?php foreach ($temp_superheroes as $superhero) : ?>
            <li><?= $superhero['alias']; ?></li>
        <?php endforeach; ?>
    </ul>

<?php endif;
    if ($hasRequestForSpecific) {
        echo json_encode($temp_superheroes);
    }
?>